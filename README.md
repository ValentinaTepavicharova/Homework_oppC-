...
Учебен проект на C++ — библиотека с книги, автори, читатели и заеми. Проектът демонстрира обектно-ориентирано програмиране (OOP) чрез класове `Author`, `Book`, `Member`, `Loan` и `Library`.
...
- [Описание на класовете](#описание-на-класовете)
- [Образователни цели](#образователни-цели)
- [Автор](#автор)
...
Отворете терминал в директорията на проекта и изпълнете:
 Примерна команда за компилация (WSL / Linux / g++):
```bash
...
Поля: std::string name, int birthYear
...
Поля: title, author (Author), year, price, isbn, static int totalBooks
...
Поля: isbn, memberId, startDate, dueDate, returned
...
Поля: вектори books, members, loans
...
Проектът демонстрира разделение между интерфейс (.h) и имплементация (.cpp)

Използва списъци за инициализация, статични членове, const-коректност и обработка на изключения.
